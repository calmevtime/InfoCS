...
def train(epochs, trainloader, valloader):
    ...
    if opt.cuda:
        reconnet = torch.nn.DataParallel(reconnet, device_ids=[1]).cuda()
        encoder = torch.nn.DataParallel(encoder, device_ids=[1]).cuda()
    ...
    for epoch in range(epochs):
        ...
        reconnet.eval()
        encoder.eval()
        val(epoch, valloader, reconnet, encoder, criterion_mse)
...
def val(epoch, valloader, net, E, criterion_mse):
