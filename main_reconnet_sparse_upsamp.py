...
parser.add_argument('--model', help='basic | adaptiveCS | adaptiveCS_resnet',
                    default='reconnet_sparse_upsamp')
...
class net(nn.Module):
    def __init__(self, opt, channels, input_size):
        ...
        self.upsamp1 = nn.Upsample(size=opt.image_size, mode='bilinear')
        self.linear1 = nn.Linear(opt.image_size ** 2, opt.image_size ** 2, bias=bias)
...
def val(opt, epoch, valloader, net, criterion_mse, image_mask, sparse_count):
    ...
    with torch.no_grad():
        for idx, (target, _) in enumerate(valloader, 0):
            ...
            img_sparse = dense_to_sparse(np.copy(target), sparse_count, image_mask, keep_dim=True)
