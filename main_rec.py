...
def main():
    ...
    if use_gpu:
        ...
        init_weights(model, init_type='normal', scale=1)
        model = nn.DataParallel(model).cuda()
        encoder = nn.DataParallel(encoder).cuda()
        criterion = nn.CrossEntropyLoss().cuda()
        optimizer = optim.SGD(list(model.parameters()) + list(encoder.parameters()), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    if args.evaluate:
        validate(testloader, encoder, model, criterion)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(trainloader, encoder, model, criterion, optimizer, epoch)

        # evaluate on test set
        prec = validate(testloader, encoder, model, criterion)
...
def train(trainloader, enc, model, criterion, optimizer, epoch):
...
def validate(val_loader, enc, model, criterion):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
