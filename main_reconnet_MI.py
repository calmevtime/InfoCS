...
def train(epochs, trainloader, valloader):
    ...
    if opt.cuda:
        reconnet = torch.nn.DataParallel(reconnet, device_ids=opt.gpu_ids).cuda(opt.device)
        encoder = torch.nn.DataParallel(encoder, device_ids=opt.gpu_ids).cuda(opt.device)
